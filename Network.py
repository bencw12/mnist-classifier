...
class Layer:
	...
	def __init__(self, n_inputs, n_neurons):
		...
		self.weights = np.random.uniform(-0.05, 0.05, (n_neurons, n_inputs))
		#Initialize biases at 0
		self.biases = np.zeros(n_neurons)
		self.change_in_weight = np.zeros((n_neurons, n_inputs))
		self.change_in_biases = np.zeros(n_neurons)

	#Feed forward one image through the layer
	def forward(self, features):

		self.z = np.dot(self.weights, features) + self.biases
...
class Network:
	...
	def update_mini_batch(self, mini_batch, lr, n, lmbda):
		...
		for i in range(len(self.layers)):
			...
			self.layers[i].change_in_weight = np.zeros(self.layers[i].weights.shape)
			self.layers[i].change_in_biases = np.zeros(self.layers[i].biases.shape)
	...
	def SGD(self, epochs, mini_batch_size, lr, lmbda):
		...
		for j in range(epochs):
			...
			training_inputs_deformed = Deformer.deform_all(self.training_images)
			training_inputs_deformed = np.reshape(training_inputs_deformed, (60000, 784))
...
def change_in_weights(error, activation):
	...
	activation = np.array([activation]).reshape(1, len(activation))

	return np.dot(error, activation)
...
def backprop_error(next_weights, next_error, this_zs):
	...
	next_error = np.array(next_error)
	this_zs = np.array(this_zs)  

	return np.dot(next_weights.T, next_error) * sigmoid_prime(this_zs)
